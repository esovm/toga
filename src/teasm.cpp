...
struct Instruction : Directive
{
    Word *a,*b;
    Instruction() : a(0), b(0) {}
    Instruction(Word *x, Word *y) : a(x), b(y) {}
    string outt() const;
    string outv(string (*)(int)) const;
...
string Instruction::outt() const 
{
	return a->outt()+' '+b->outt() + '\t' + "\t# " + tos(a->addr); 
}
string Instruction::outv(string (*f)(int)) const { return a->outv(f)+' '+b->outv(f); }
...
void usage(int r)
{
    cout<<"Useage: teasm [-b] [-t] [-c] input.te [output.run]\n";
...
bool getstatement(istream &is, string &s)
{
	s="";
	while(1)
	{
...
vector<Directive*> readcc(istream &is, map<string,Directive*> * conds)
{
	...
	{
		s = s.substr(0,i);
		if( !endln ){ string x; std::getline(is,x); }
	}
...
pair<bool,Instruction*> getinstr(const string &s)
{
    ...
    ret.second = new Instruction(a.second,c.second);
